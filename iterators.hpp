 ...
 // ============= BITS STL ITERATOR =============
						...
						typename iterator_traits<Iterator>::reference>
	{
		...
		public:
			 ...
			 */
			operator*() const { Iterator tmp = current; return (*--tmp); }
			 ...
			 */
			operator->() const
			{ Iterator tmp = current; --tmp; return _S_to_pointer(tmp); }
		...
		private:
			template<typename Tp>
			static Tp* _S_to_pointer(Tp*_p) { return p; }

			template<typename Tp>
			static pointer _S_to_pointer(Tp t) { return t.operator->(); }
    };
